...
# 0.4.4: ONESR con relleno morado y contorno negro real.
# Se usa draw_string_outline() para contornear el glifo sin desplazar copias
# del texto hacia el interior de la propia tipografia.
...
func _draw_intro() -> void:
	...
	_comm_center("PIXEL ADVENTURE - PROTOTIPO 0.4.4", 164, Color("555564"), 6)
...
func _draw_important_dialogue() -> void:
	...
	# Sin nombre redundante del NPC. ONESR: morado dentro, negro fuera.
	var combined := ""
...
func _npc_text(text: String, x: float, y: float, color: Color, size: int) -> void:
	# draw_string_outline genera un borde alrededor de la silueta real del glifo.
	# Esto evita el efecto anterior en el que las ocho copias negras invadian
	# el interior de ONESR y hacian parecer que morado y negro estaban invertidos.
	draw_string_outline(
		DIALOGUE_FONT,
		Vector2(x, y),
		text,
