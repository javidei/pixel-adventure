...
const UI_FONT_PATH := "res://assets/fonts/Windows Regular.ttf"
const DIALOGUE_FONT_PATH := "res://assets/fonts/ONESR___.TTF"
...
const CARTOGRAPHER_DIALOGUE: Array[Dictionary] = [
	{
		"speaker": "CARTOGRAFO",
		"line1": "ASI QUE TU ERES EL QUE HA LLEGADO",
		"line2": "A NARANJAL DEL RIO."
	},
	{
		"speaker": "PROTAGONISTA",
		"line1": "DEPENDE. QUIEN QUIERE SABERLO?",
		"line2": ""
	},
	{
		"speaker": "CARTOGRAFO",
		"line1": "ALGUIEN QUE SABE QUE ESTE PUEBLO",
		"line2": "NO APARECE EN LOS MAPAS POR CASUALIDAD."
	},
	{
		"speaker": "PROTAGONISTA",
		"line1": "Y QUE SE SUPONE QUE TENGO QUE BUSCAR?",
		"line2": ""
	},
	{
		"speaker": "CARTOGRAFO",
		"line1": "EMPIEZA POR LO QUE ESTA TORCIDO.",
		"line2": "A VECES LO IMPORTANTE ESTA A PLENA VISTA."
	}
]
...
const COL_DIALOGUE_BG := Color(0.025, 0.03, 0.055, 0.96)

var ui_font: Font = ThemeDB.fallback_font
var dialogue_font: Font = ThemeDB.fallback_font
...
func _ready() -> void:
	...
	dialogue_font = _load_font(DIALOGUE_FONT_PATH)
	_load_room()
	queue_redraw()
...
func _process(delta: float) -> void:
	...
	if not dialogue_active and absf(player_x - player_target_x) > 0.1:
	...
	if not dialogue_active and not pending_hotspot_id.is_empty() and absf(player_x - player_target_x) <= 0.75:
	...
	if moved or dialogue_active or int(Time.get_ticks_msec() / 180) % 2 == 0:
		queue_redraw()
...
func _input(event: InputEvent) -> void:
	...
	if dialogue_active:
		if event is InputEventMouseButton:
			var mouse_dialogue := event as InputEventMouseButton
			if mouse_dialogue.button_index == MOUSE_BUTTON_LEFT and mouse_dialogue.pressed:
				_advance_dialogue()
				accept_event()
		elif event is InputEventKey:
			var key_dialogue := event as InputEventKey
			if key_dialogue.pressed and not key_dialogue.echo and (key_dialogue.keycode == KEY_SPACE or key_dialogue.keycode == KEY_ENTER):
				_advance_dialogue()
				accept_event()
		elif event is InputEventScreenTouch:
			var touch_dialogue := event as InputEventScreenTouch
			if touch_dialogue.pressed:
				_advance_dialogue()
				accept_event()
		return
...
func _start_cartographer_dialogue() -> void:
	current_dialogue = CARTOGRAPHER_DIALOGUE.duplicate(true)
	dialogue_index = 0
...
func _advance_dialogue() -> void:
	...
	if dialogue_index >= current_dialogue.size():
		...
		message = "EL CARTOGRAFO SE QUEDA MIRANDO EL MAPA TORCIDO."
	queue_redraw()
...
func _draw_intro() -> void:
	...
	_text_center("PIXEL ADVENTURE · PROTOTIPO 0.2.1", 164, Color("555564"), 6)
...
func _draw_player() -> void:
	var moving := absf(player_x - player_target_x) > 0.9 and not dialogue_active
...
func _draw_hotspot_highlight() -> void:
	...
	if typeof(rect_value) == TYPE_RECT2:
		...
		var screen_rect := Rect2(
			world_rect.position.x - camera_x,
...
func _draw_dialogue() -> void:
	if current_dialogue.is_empty() or dialogue_index >= current_dialogue.size():
		return

	var entry: Dictionary = current_dialogue[dialogue_index]
	var speaker := str(entry.get("speaker", ""))
	var line1 := str(entry.get("line1", ""))
	var line2 := str(entry.get("line2", ""))

	draw_rect(Rect2(7, 69, 306, 41), COL_DIALOGUE_BG)
	draw_rect(Rect2(7, 69, 306, 41), COL_GOLD, false, 1.0)

	_dialogue_text(speaker, 13, 81, COL_GOLD, 7)
	_dialogue_text(line1, 13, 94, COL_TEXT, 7)
	if not line2.is_empty():
		_dialogue_text(line2, 13, 104, COL_TEXT, 7)

	if int(Time.get_ticks_msec() / 480) % 2 == 0:
		_dialogue_text(">", 301, 104, COL_CYAN, 7)
