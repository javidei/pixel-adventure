...
# 0.4.3: dialogos NPC compactos, sin nombre redundante, con ONESR
# en relleno morado + contorno negro y ajuste automatico de lineas.

const NPC_DIALOGUE_FILL := Color("c487c6")
const NPC_DIALOGUE_OUTLINE := Color("080608")
...
func _draw_intro() -> void:
	...
	_comm_center("PIXEL ADVENTURE - PROTOTIPO 0.4.3", 164, Color("555564"), 6)
...
func _draw_important_dialogue() -> void:
	# Escena/retrato arriba y respuestas del protagonista abajo.
	...
	# El nombre del NPC no se repite: el retrato ya identifica al interlocutor.
	var combined := ""
	...
	# ONESR mas pequena y envuelta para no salir nunca del margen derecho.
...
func _npc_text(text: String, x: float, y: float, color: Color, size: int) -> void:
	# ONESR directa. La fuente completa ya esta validada en CI.
	# El contorno se pinta en ocho direcciones para que sea negro real,
	# no una simple sombra desplazada.
	var offsets := [
	...
	for offset in offsets:
		draw_string(DIALOGUE_FONT, Vector2(x, y) + offset, text, HORIZONTAL_ALIGNMENT_LEFT, -1.0, size, NPC_DIALOGUE_OUTLINE)
...
func _wrap_dialogue_text(text: String, max_width: float) -> Array[String]:
	...
	for word in words:
		...
		var width := DIALOGUE_FONT.get_string_size(candidate, HORIZONTAL_ALIGNMENT_LEFT, -1.0, NPC_DIALOGUE_SIZE).x
